...
%	Run down of main program flow. Reference appendix for flowchart
	...
	%the median is 1, and 0 otherwise. This can be efficiently implemented by creating an n by n mask filled 
	%with ones, and convolving it with binary mask to create a new image with each pixel containing the sum of
	%its neighbours. The image is then thresholded at 4.5, causing any pixel who had more 1 than 0
	%neighbours to be set to white, and any with more 0 neighbours to be set to black, which would

...
Contrary to popular belief, skydiving is a competitive and technical sport, requiring careful control of body position in order to not only remain stable, but also to move around the sky in free-fall. A popular discipline in competitive skydiving is formation skydiving, where a group of skydivers form set shapes with their bodies whilst in free-fall. It is proposed that a software tool will be written that is able to automatically judge video footage of formation skydiving competitions. This is done by analysing the pose of each skydiver individually, in order to fit a stick man model. The interaction between these models is then used to find the overall shape of the formation, enabling formation detection. The final goal is to use this tool to remove the need for manual competition judging.
}}\end{abstract}
...
However, there does exist literature relevant to active shape models\cite{cootes} and posture recognition.
...
%
\subsection{Posture Recognition}
%
Human pose detection can be a difficult problem to tackle; many factors contribute towards this, such as shading, occlusions and low image quality.\\
Existing techniques fall into two main categories, model-based and learning-based algorithms. Model-based algorithms assume some explicit prior knowledge of a body model, often requiring accurate 3D modelling and rendering. Learning-based algorithms attempt to remove this need often by iteratively collecting data and using that to create a model.
